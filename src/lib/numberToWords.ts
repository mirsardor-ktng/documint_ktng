...
function numberToWordsRu(num: number): string {
    ...
    if (num < 0) {
        // Handle negative if necessary, though context implies positive amounts
         return 'Минус ' + numberToWordsRu(Math.abs(num));
    }
    if (isNaN(num)) {
...
function convertLessThanOneThousandEn(num: number): string {
    if (num === 0) return '';
    let words = '';
    if (num >= 100) {
        words += onesEn[Math.floor(num / 100)] + ' hundred';
        num %= 100;
        if (num > 0) words += ' ';
    }
    if (num >= 20) {
        words += tensEn[Math.floor(num / 10)];
        num %= 10;
        if (num > 0) words += ' ' + onesEn[num]; // Using space instead of hyphen for consistency
    } else if (num >= 10) {
        words += teensEn[num - 10];
    } else if (num > 0) {
        words += onesEn[num];
    }
    return words;
...
function numberToWordsEn(num: number): string {
    if (num === 0) return 'Zero sum 00 tiyin';
    ...
    if (integerPart === 0) {
        ...
        do {
            const chunk = integerPart % 1000;
            if (chunk !== 0) {
                const chunkWords = convertLessThanOneThousandEn(chunk);
                integerWords = chunkWords + (thousandsEn[i] ? ' ' + thousandsEn[i] : '') + (integerWords ? ' ' + integerWords : '');
            }
            integerPart = Math.floor(integerPart / 1000);
            i++;
        } while (integerPart > 0);
    ...
    const sumWord = Math.floor(num) === 1 ? 'sum' : 'sums';
    const decimalFormatted = decimalPart.toString().padStart(2, '0');
    const tiyinWord = decimalPart === 1 ? 'tiyin' : 'tiyins';

    const finalWords = `${integerWords.trim()} ${sumWord} ${decimalFormatted} ${tiyinWord}`;
    return finalWords.charAt(0).toUpperCase() + finalWords.slice(1);
}

// --- Dispatcher ---

export function numberToWords(num: number, lang: string = 'ru'): string {
    const langCode = lang ? lang.toLowerCase() : 'ru';
    switch (langCode) {
        case 'en':
        case 'eng':
